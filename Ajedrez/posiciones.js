var B,i,y,u,b,I=[],G=120,x=10,z=15,M=1e4,l=[5,3,4,6,2,4,3,5,1,1,1,1,1,1,1,1,9,9
		...
		y^=8;G++;d=w||s&&s>=h&&X(0,0,0,21,0,0)>M;

		do{if(o=I[p=O]){q=o&z^y;if(q<7){A=q--&
		2?8:4;
		C=o-9&z?[53,47,61,51,47,47][q]:57;

		do{r=I[p+=l[C]];if(!w|p==w){g=q|p+a-S?0
		:S;
		if(!r&(!!q|A<3||!!g)||(r+1&z^y)>9&&q|A>2){if(m=!(r-2&7))
		return y^=8,I[G--]=
		O,K;
		J=n=o&z;E=I[p-a]&z;t=q|E-7?n:(n+=2,6^y);
		while(n<=t){L=r?l[r&7|32]-h-q:0;
		...
		{I[p]=n,I[O]=m?(I[g]=I[m],I[m]=0):g?I[g]=0:0;L-=X(s>h|d?0:p,L
		-N,h+1,I[G+1],J=q|A>1?0:p,s);if(!(h||s-1|B-O|i-n|p-b|L<-M))
		return W(),G--,u=J;
		J=q-1|A<7||m||!s|d|r|o<z||X(0,0,0,21,0,0)>M;
		I[O]=o;I[p]=r;m?(I[m]=I[g],I[g]=0):
		...
		"<th width=50 height=50 onclick=Y("+i+") id=o"+i+

		" style='line-height:40px;font-size:40px;border:1px solid #dde' bgcolor=#"+

		(i*.9&1?"c0c":"f0f")+"0f0>":(i++,"<tr>"));
		a+="<th colspan=8><select id=t style='font-size:20px'><option>â™›<option>";
		document.write(a+"**");
		function W(){B=b;

		for(p=21;p<99;++p)if(q=document.getElementById("o"+p)){q.

		innerHTML="\xa0\u265f\u265a\u265e\u265d\u265c\u265b  \u2659\u2654\u2658\u2657\u2656\u2655".charAt(I[p]&z);

		q.style.borderColor=p==B?"red":"#dde";}}W();
		function Y(s){i=(I[s]^y)&z;if(i>8){b=s;W();}

		else if(B&&i<9){b=s;i=I[B]&z;if((i&
